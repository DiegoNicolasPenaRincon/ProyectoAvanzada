...
//    }
        ...
        return loadTestData(
                List.of(
                        new Usuario(UUID.randomUUID().toString(),"ana@example.com", "{noop}12346Abc",
                                "Ana López", Rol.USER, UsuarioEstado.ACTIVO),
                        new Usuario(UUID.randomUUID().toString(),"carlos@example.com","{noop}12346Abc",
                                "Carlos Pérez",  Rol.USER, UsuarioEstado.ACTIVO),
    ...
    public static Map<String, Usuario> loadTestData(Collection<Usuario> newUsers,UsuarioRepository userRespository, MongoTemplate mongoTemplate) {
        // Borrar datos existentes para asegurar la repetibilidad de las pruebas.
        mongoTemplate.getDb().listCollectionNames()
                .forEach(mongoTemplate::dropCollection);
        return userRespository.saveAll(newUsers).stream().collect(Collectors.toMap(Usuario::getId, usuario -> usuario));
    }
}